...
The morning review (internal:34) completed after the post-restart re-dispatch and its findings
...
## What Was Planned (from today's morning review, internal:34)

The morning review (job:morning-review, status: done) assessed the state of the system after
the service outage and flagged:

1. **#441** — `orch task unblock` ignores internal task IDs — still open
2. **#448** — Engine health checks blind to internal tasks — still open
3. **#446** — `orch task status` excludes internal tasks — in progress
4. Reduce stuck detection thresholds (part of #448) — not done
5. internal:29 code-development loop — analysis-only pattern, job body needs directive language

**Completed from morning review priorities**: internal:31 (throttle) and internal:32 (agent logs)
were already merged by the time the morning review ran. The morning review correctly noted these
as done and shifted focus to the internal:29 analysis-only loop.

**Not completed**: #441, #448, #446, and threshold reduction remain open. The outage consumed the
available agent capacity for the day.
...
| internal:33 | Code review: orch | done | Completed post-restart re-dispatch |
| internal:34 | Morning review | done | Completed post-restart re-dispatch |
...
and implement it, not analyze it. This was applied as a live edit to `.orch.yml` (not committed
to this PR — only the retrospective file is tracked here).
...
and implement it in full, including tests and commit. Applied as a live edit to `.orch.yml`.
...
## Open GitHub Issues (last known state from morning review, internal:34)
...
4. **internal:29** — with the updated job body, the next code-development run should produce
   actual code. Watch attempt #8 closely.
5. **Monitor #446** — `orch task status` internal task visibility is in progress; verify it lands.
