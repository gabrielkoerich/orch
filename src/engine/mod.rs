...
async fn cleanup_worktrees(
    ...
    for task in &tasks {
        ...
        match std::fs::remove_dir_all(&worktree_path) {
            Ok(()) => {
                let _ =
                    crate::sidecar::set(&task.id.0, &["worktree_cleaned=1".to_string()]);
                cleaned += 1;

                // Delete local branch from the bare clone
                if git_dir.exists() {
                    if let Err(e) = gh.delete_local_branch(&git_dir, &branch).await {
                        tracing::debug!(branch, ?e, "could not delete local branch (may not exist)");
                    }
...
async fn check_merged_prs(
    ...
    for task in &in_review {
        ...
        let branch = match crate::sidecar::get(&task.id.0, "branch") {
            Ok(b) if !b.is_empty() => b,
            _ => {
                tracing::debug!(task_id = task.id.0, "no branch in sidecar, skipping PR check");
                continue;
            }
        };
