        ...
        match parse_result {
            Ok(parsed) => {
                ...
                if resp.status == "done" || resp.status == "in_progress" {
                    ...
                    // Push
                    let push_ok = match git_ops::push_branch(
                        &wt.work_dir,
                        &wt.branch,
                        &wt.default_branch,
                    )
                    .await
                    {
                        Ok(true) => true,
                        Ok(false) => {
                            tracing::error!(task_id, "push failed (non-zero exit), skipping PR creation");
                            sidecar::set(task_id, &["last_error=push failed".to_string()])?;
                            false
                        }
                        Err(e) => {
