...
mod tests {
    ...
    fn test_orch_home_creates_directory() {
        ...
        std::fs::create_dir(&home).expect("should create home dir");

        let orch_path = home.join(HOME_DIR);
        std::fs::create_dir_all(&orch_path).expect("should create orch path");
    ...
    fn test_state_dir() {
        ...
        std::fs::create_dir_all(&state).expect("should create state dir");
    ...
    fn test_task_dir_creates_path() {
        ...
        let dir = task_dir("test-owner/test-repo", "42").expect("task_dir should succeed");
        assert!(dir.exists());
    ...
    fn test_task_attempt_dir_creates_path() {
        ...
        let dir = task_attempt_dir("test-owner/test-repo", "42", 1)
            .expect("task_attempt_dir should succeed");
        assert!(dir.exists());
    ...
    fn test_repo_state_dir_separates_repos() {
        ...
        let dir_a =
            repo_state_dir("owner/repo-a").expect("repo_state_dir should succeed for repo-a");
        let dir_b =
            repo_state_dir("owner/repo-b").expect("repo_state_dir should succeed for repo-b");
