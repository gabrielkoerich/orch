...
mod tests {
    ...
    fn test_task_dir_creates_path() {
        ...
        let temp = TempDir::new().unwrap();
        let home = temp.path();
        // Set HOME using a string to avoid platform-dependent AsRef conversions
        // which can be sensitive during parallel test runs.
    ...
    fn test_task_attempt_dir_creates_path() {
        ...
        let temp = TempDir::new().unwrap();
        let home = temp.path();
        // Use a UTF-8 string representation for HOME to avoid intermittent
        // failures when multiple tests manipulate the environment concurrently.
        std::env::set_var("HOME", home.to_str().unwrap());
